...
class BaseNetAux(nn.Module):
    ...
    def forward(self, x):
        ...
        y_2 = F.max_pool2d(self.conv1_1(x[:, 1].view(-1, 1, 14, 14)), kernel_size = 2)
        ...
        y_1 = F.max_pool2d(self.conv2_1(y_1), kernel_size = 2)
        y_2 = F.max_pool2d(self.conv2_2(y_2), kernel_size = 2)
        if self.batch_normalization:
            y_1 = self.bn2_1(y_1)
            y_2 = self.bn2_2(y_2)
        y_1 = F.relu(y_1)
        ...
        y_1 = self.fc1_1(y_1.view(-1, 256))
        y_2 = self.fc1_1(y_2.view(-1, 256))
        if self.batch_normalization:
            y_1 = self.bn3_1(y_1)
            y_2 = self.bn3_2(y_2)
        y_1 = F.relu(self.dropout(y_1))
        y_2 = F.relu(self.dropout(y_2))

        y_1_loss = self.fc2_1(y_1)
        y_2_loss = self.fc2_2(y_2)
        if self.batch_normalization:
            y_1_loss = self.bn4_1(y_1_loss)
            y_2_loss = self.bn4_2(y_2_loss)
        y_1 = F.relu(y_1_loss)
        y_2 = F.relu(y_2_loss)
        y = self.fc3(torch.cat((y_1, y_2), 1))
